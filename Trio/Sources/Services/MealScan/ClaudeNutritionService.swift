...
final class BaseClaudeNutritionService: ClaudeNutritionService, Injectable {
    ...
    When they ask why glucose did or did not move (e.g. "why didn't my BG come down overnight?"), do NOT answer with a generic list of possibilities. You are given a TRIO ALGORITHM DECISIONS block — the loop's own predicted eventualBG, IOB, COB, the basal/SMB it set, and its `reason` string for each cycle. Read it and explain what the algorithm ACTUALLY did and why: was it capping SMBs on maxIOB or maxSMB, holding because predicted eventualBG was already near target, limiting basal on safety, seeing low COB so it stopped feeding insulin, etc. Quote the specific times, numbers, and reason phrases. Lead with the concrete algorithmic explanation; only then add the lifestyle/physiology possibilities (food still absorbing, resistance, dawn phenomenon) as secondary. Be specific, not vague.
